...
function initGrid() {
	...
	$('#mrpGatheringGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local', 
		colNames : [ "소요량취합번호", "구매 및 생산여부", "품목코드", "품목명", "단위", 
			"지시일", "납기일", "필요수량", "공정진행여부"], 
		colModel : [
			...
			{ name: "claimDate", width: "70", resizable: true, align: "center", editable: true,
				formatter: 'date',  
				formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' }
			} ,
			{ name: "dueDate", width: "70", resizable: true, align: "center", editable: true,
				formatter: 'date',  
				formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' }
			} ,
			{ name: "necessaryAmount", width: "60", resizable: true, align: "center"} ,
			{ name: "onGoingProcessStatus", width: "100", resizable: true, align: "center"}
	...
	$('#mrpSearchGrid').jqGrid({ 
		...
		colModel : [
			...
			{ name: "itemName", width: "120", resizable: true, align: "center"} ,
			{ name: "orderDate", width: "70", resizable: true, align: "center", editable: true,
				formatter: 'date',  
				formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' }
			} ,
			{ name: "requiredDate", width: "70", resizable: true, align: "center", editable: true,
				formatter: 'date',  
				formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' }
			} ,
...
function showMrpSearchGrid() {
	...
	// mrpGatheringGrid 를 클릭할 때마다 소요량 전개 Data 를 따로 불러오는 경우의 ajax
	...
	$.ajax({ 
		...
		data : {
			method : 'getMrpList', 
			mrpGatheringNo : selectedMrpGatheringNo
		},
		dataType : 'json', 
		cache : false, 
		success : function(dataSet) { 
