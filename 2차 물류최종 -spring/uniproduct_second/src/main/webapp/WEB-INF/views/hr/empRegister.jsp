...
$(document).ready(function() {
	...
	$("input[type=button], input[type=submit], input[type=file]").button(); // jqueryUI Button 위젯 적용
	...
	$('#empBasicInfoGrid').addRowData( 1, { status : "INSERT" , userOrNot : "Y" , companyCode : "${sessionScope.companyCode}" } );

	$('#empDetailInfoGrid').addRowData( 1, { status : "INSERT" , companyCode : "${sessionScope.companyCode}" , seq : "1" , updateHistory : "신규가입" } );
...
function initGrid() {
	...
	// 사원 기본정보 그리드 시작
	$('#empBasicInfoGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "사원코드", "사원명", "사원영문명", "주민등록번호", "입사일자" , "생년월일" , "성별", "status" , "userOrNot" , "companyCode" ],
		colModel : [
			{ name: "empCode", width: "80", resizable: true, align: "center" } ,
			{ name: "empName", width: "80", resizable: true, align: "center" , editable: true } ,
			{ name: "empEngName", width: "140", resizable: true, align: "center" , editable: true } ,
			{ name: "socialSecurityNumber", width: "120", resizable: true, align: "center" , editable: true } ,
			{ name: "hireDate", width: "70", resizable: true, align: "center", editable: true,
						...
						dataInit: function (element) { 
							$(element).datepicker({ 
								...
								onClose: function(dateText, datepicker) {
									$('#empBasicInfoGrid').editCell(1, 5, false); 
								}
		                  })}
		          }, 
		          editrules: { date: true } 
			} ,
			{ name: "birthDate", width: "70", resizable: true, align: "center", editable: true,
//				  formatter: 'date',   => 주석 처리 : 여기 지정되면 사용자가 값을 미입력시 걸러주지 못함
//				  formatoptions: { srcformat: 'ISO8601Long', newformat: 'Y-m-d' },  
				  edittype: 'text', 
		          editoptions: { size: 12, maxlengh: 12, 
						dataInit: function (element) { 
							$(element).datepicker({ 
								...
								numberOfMonths: 1, 
								onClose: function(dateText, datepicker) {
									$('#empBasicInfoGrid').editCell(1, 6, false); 
								}
		                  })}
		          }, 
		          editrules: { date: true } 
			} ,
			{ name: "gender", width: "70", resizable: true, align: "center",editable : true },
			{ name: "status", width: "80", resizable: true, align: "center" },
			{ name: "userOrNot", width: "80", resizable: true, align: "center" , hidden : true	},
			{ name: "companyCode", width: "70", resizable: true, align: "center" , hidden : true } 
		], 
		caption : '사원 기본정보 등록',
		sortname : 'seq', 
		multiselect : false, 
	...
	$('#empDetailInfoGrid').jqGrid({ 
		mtype : 'POST', 
		datatype : 'local',
		colNames : [ "사업장코드", "부서코드", "직급코드", "사용자ID", "비밀번호", 
			"전화번호", "email", "우편번호", "기본주소", "상세주소", "최종학력", "status" ,
			"companyCode", "empCode", "seq", "updateHistory", "updateDate" , "image" ], 
		colModel : [

			{ name: "workplaceCode", width: "75", resizable: true, align: "center" } ,
			{ name: "deptCode", width: "75", resizable: true, align: "center" } ,
			{ name: "positionCode", width: "75", resizable: true, align: "center" } ,
			{ name: "userId", width: "75", resizable: true, align: "center" } ,
			{ name: "userPassword", width: "75", resizable: true, align: "center" } ,
			{ name: "phoneNumber", width: "100", resizable: true, align: "center" , editable : true } ,
			{ name: "email", width: "120", resizable: true, align: "center" , editable : true } ,
			{ name: "zipCode", width: "80", resizable: true, align: "center" } ,
			{ name: "basicAddress", width: "300", resizable: true, align: "center" } ,
			{ name: "detailAddress", width: "140", resizable: true, align: "center", editable: true } ,
			{ name: "levelOfEducation", width: "75", resizable: true, align: "center" , editable : true } ,
			{ name: "status", width: "80", resizable: true, align: "center" } ,
			{ name: "companyCode", width: "90", resizable: true, align: "center" , hidden: true }, 
			{ name: "empCode", width: "70", resizable: true, align: "center" , hidden: true },
			{ name: "seq", width: "70", resizable: true, align: "center" , hidden: true },
			{ name: "updateHistory", width: "90", resizable: true, align: "center" , hidden: true },		
			{ name: "updateDate", width: "90", resizable: true, align: "center" , hidden: true } ,
...
function initEvent() {
	...
	$('#checkEmpCodeDuplicationButton').on("click" , function () {

		checkEmpCodeDuplicationStatus = false;
		
		var newEmpCodeValue = $('#empCodeInputBox').val().trim();
		var duplicated;
	...
	$('#batchSaveButton').on("click" , function () {
		...
		var empBasicInfo = $('#empBasicInfoGrid').getRowData(1);
		var empDetailInfo = $('#empDetailInfoGrid').getRowData(1);
		
		for(var key in empBasicInfo) {
			if( empBasicInfo[key] == '' || empBasicInfo[key] == null ) {
				
				alertError("사용자 에러" , "사원 기본 정보 중에서 입력하지 않은 값이 있습니다")
				return;
				
			}
		}		
		
		if( empDetailInfo.userId == '' || empDetailInfo.userPassword == '' ) {
			
			alertError("사용자 에러" , "사원 세부 정보 중에서 사용자 ID 와 비밀번호는 필수 입력 사항입니다")
			return;
		...
		if( confirmStatus == true ) {
			...
			var empSecretInfo = 
				{ 
					companyCode : "${sessionScope.companyCode}" , empCode : empBasicInfo.empCode ,
					seq : 1 , userPassword : empDetailInfo.userPassword , status : "INSERT"
				};
...
function showCodeDialog(grid, rowid, iCol, divisionCodeNo, title) {
	...
	$("#codeGrid").jqGrid({
		...
		colModel : [ 
			...
			{ name : 'detailCodeName', width : 140, align : "center", editable : false }, 
			{ name : 'codeUseCheck', width : 80, align : "center", editable : false} 
		],
		width : 400,
		height : 300,
		caption : title,
		...
		buttons : {  // 버튼 이벤트 적용
			"확인" : function() {
				...
				if( checkUserIdDuplicationStatus == false ) {
					...
					alertError("사용자 에러" , "중복된 ID 이거나 중복체크를 하지 않았습니다. </br> 값을 새로 입력해 주세요");
					
				} else if ( $('#userPasswordInputBox').val() == '' ) {
				
					alertError("사용자 에러" , "비밀번호를 입력하지 않았습니다. </br> 값을 새로 입력해 주세요");
				
				} else if ( checkUserIdDuplicationStatus  == true ) {
					
					$(grid).setCell(rowid, 3, $('#userIdInputBox').val().trim() );
					$(grid).setCell(rowid, 4, $('#userPasswordInputBox').val().trim() );
		...
		buttons : {  // 버튼 이벤트 적용
			"확인" : function() {
				...
				if( checkEmpCodeDuplicationStatus == false ) {
					...
					alertError("사용자 에러" , "중복된 사원코드이거나 중복체크를 하지 않았습니다. </br> 값을 새로 입력해 주세요");
					
				} else if ( checkEmpCodeDuplicationStatus == true ) {
					
					$(grid).setCell(rowid, iCol, $('#empCodeInputBox').val().trim() );
					checkEmpCodeDuplicationStatus = false;
				}
...
<fieldset id="searchConditionFieldset"  style="width: 10%; display: inline-block; vertical-align: top;">
	<legend>사원 등록</legend>
		<input type="button" value="새로운 사원 정보 저장" id="batchSaveButton" />
			
</fieldset>

<fieldset id="addNewEmpPhotoFieldset"  style="width: 20%; height : 60px; display: inline-block; vertical-align: top;">
	<legend>사진 등록</legend>
...
<div id="checkUserIdDialog">
	<table>
		...
		<tr>
			<td><label for="userPasswordInputBox" style="font-size: 20px; margin-right: 10px">비밀번호</label></td>
			<td>
				<input type="text" id="userPasswordInputBox"/>&nbsp;&nbsp;
			</td>
		</tr>
	</table>
	...
	<table>
		<tr>
			<td>
				...
				<label for="empCodeInputBox" style="font-size: 20px; margin-right: 10px">새로운 사원코드</label>
				<input type="text" id="empCodeInputBox"/>&nbsp;&nbsp;
				
			</td>
			<td>
				<input type="button" value = "사원 코드 중복 체크" id="checkEmpCodeDuplicationButton" style="font-size: 16px;"/>
			</td>
		</tr>
		<tr>
			<td></td>
			<td>
				<input type="button" value = "사원 코드 자동 생성" id="makeNewEmpCodeButton" style="font-size: 16px;"/>
			</td>
		</tr>
